...
def build(args):
    ...
    state_dict = torch.load(
        "./Trained models/DuaPSNet_{}.pt".format(args.train_dataset)
    )
...
def predict(args):
    ...
    for i, (data, target) in enumerate(test_dataloader):
        ...
        # predicted_map = predicted_map > 0.5
        # cv2.imwrite(
        #     "./Predictions/Trained on {}/Tested on {}/{}".format(
        ...
        predicted_map = (predicted_map * 255).astype(np.uint8)
        cv2.imwrite(
            "./Predictions/Trained on {}/Tested on {}/{}".format(
                args.train_dataset, args.test_dataset, os.path.basename(target_paths[i])
            ),
...
#     )
#     parser.add_argument(
#         "--train-dataset", type=str, required=True, choices=["Kvasir", "CVC", "Mix"]
#     )
#     parser.add_argument(
#         "--test-dataset", type=str, required=True, choices=["Kvasir", "CVC", "Mix"]
#     )
