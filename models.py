...
import numpy as np
class Adaptive_Feature_Aggregation_Module(nn.Module):
    def __init__(self, in_channels_F2, in_channels_F3, in_channels_F4, *args, **kwargs):
        super(Adaptive_Feature_Aggregation_Module, self).__init__(*args, **kwargs)
...
class Selective_Global_to_Local_Fusion_Head_Module(nn.Module):
    def __init__(self, in_channels1, in_channels2):
        super(Selective_Global_to_Local_Fusion_Head_Module, self).__init__()
...
class TB(nn.Module):
    def __init__(self):
        ...
        self.AFA = Adaptive_Feature_Aggregation_Module(128, 320, 512)
        self.RB = ResidualBlock(64, 64)
        self.SGLFH = Selective_Global_to_Local_Fusion_Head_Module(64, 64)
    ...
    def forward(self, x):
        ...
        M1_out = self.AFA(pyramid[1], pyramid[2], pyramid[3])

        O2 = self.RB(pyramid[0])

        output = self.SGLFH(M1_out, O2)
